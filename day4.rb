...
class Room
  def initialize(encrypted_name)
    ...
    @name = encrypted_name[0..(encrypted_name.rindex('-')-1)]
    @sector_id = encrypted_name[(encrypted_name.rindex('-')+1)..(encrypted_name.index('[')-1)].to_i
  ...
  def real?
    ...
    letters.each_char do |char|
      if occurrences[char] == nil
        occurrences[char] = 1
      else
        occurrences[char] += 1
      end
    end
    checksum = occurrences.sort_by { |k, v|
      # 1 - 26 allocated to 1 occurrence
      # 27 - 52 allocated to 2 occurrences
      # etc.
  ...
  def decrypted_name
    decr_name = String.new
    encr_name = @name.gsub '-', ' '
    encr_name.each_char do |char|
      cp = char.codepoints[0]
      if cp >= 97 and cp <= 122
        decr_name += ''.concat(97 + ((char.codepoints[0] - 97 + (@sector_id % 26)) % 26))
      else
        decr_name += char
      end
    end
...
puts "Sum of sector IDs: #{total_sector_id}"
